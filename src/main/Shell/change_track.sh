...
for i in 1 2 4 8 16 32; do
	echo ${i}
	echo "Start ${document_count}数据量,${i}gheap的第${x} 次esrally at :"`date +"%Y-%m-%d %H:%M:%S"` >> /usr/local/esrally.log
	esrally race --pipeline=benchmark-only --target-hosts=192.168.100.48:9200,192.168.100.49:9200,192.168.100.50:9200,192.168.100.51:9200 --track-path=/usr/local/esrally/benchmarks/tracks/enadmin/ --car="${i}gheap"  --user-tag="data:${document_count},car:${i}gheap"
	echo "End  ${document_count}数据量,${i}gheap的第${x} 次esrally at:"`date +"%Y-%m-%d %H:%M:%S"` >> /usr/local/esrally.log
done
